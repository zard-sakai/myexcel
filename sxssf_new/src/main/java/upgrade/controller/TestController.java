...
import upgrade.pojo.ExcelColumnInfo;
...
public class TestController {
    ...
    public static final String gpSql = "select column1,column2,column3,column4 from public.test t limit 1000";
    ...
    /**
     * http://localhost:8080/myexcel/example/sxssf2
     * 一次性将gp数据全部拉取到内存，不做分页
     */
...
//        ResultSet resultSet = executeMysqlSql(sql);
        ...
        workbook = ExcelExportUtil.exportBigExcel(new ExportParams(null, null), Map.class, new IExcelExportServer() {
            @Override
            public List<Object> selectListForExcelExport(Object queryParam, int page) {
                // queryParam可以传递dao层对象，将条件封装在该对象中，page用来做分页
//                Integer pageSize = DEFAULT_MAX_ROW;
                if(page > 1){//当前数据量较少，执行一次就结束循环，后续切换到sxssf+stream模式
                    dataList.clear();//结束递归
                }
        ...
        if (resultNum < 1) {
            ...
            logger.info("执行excel报表sql查询导出数据行数:{}",resultNum);
        }
    ...
    /**
     * http://localhost:8080/myexcel/example/sxssf3
     * 分页将gp数据全部拉取到内存
     */
    @GetMapping("/myexcel/example/sxssf3")
    public void sxssfMode3(HttpServletResponse response) throws Exception {
        // get column excel name
        List<ExcelColumnInfo> excelColumnInfoList = generateExcelColumnInfoList(jsonColumn);
        List<ExcelExportEntity> excelExportEntityList = generateExcelExportEntity(excelColumnInfoList);
...
//        ResultSet resultSet = executeMysqlSql(sql);
        ...
        while (resultSet.next()) {
            ...
            workbook = ExcelExportUtil.exportBigExcel(new ExportParams(null, null), Map.class, new IExcelExportServer() {
                ...
                public List<Object> selectListForExcelExport(Object queryParam, int page) {
                    // queryParam可以传递dao层对象，将条件封装在该对象中，page用来做分页
                    int pageSize = (Integer)queryParam;
                    if(page > 1){//当前数据量较少，执行一次就结束循环
                        dataList.clear();
                    }
                    return Lists.newArrayList(dataList.toArray());
                }
